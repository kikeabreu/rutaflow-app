...
describe("TOUR_STEPS", () => {
  ...
  test("los pasos de práctica traen la instrucción visible", () => {
    const practica = TOUR_STEPS.filter(s => s.action === "click");
  ...
  test("todo paso con práctica apunta a un ancla real", () => {
    ...
    const anclas = new Set([
      "variables", "platforms", "ai-chat", "stats-cards", "trips-list",
      "nuevo-viaje", "registro-rapido", "jornada-card", "copilot-card", "kpi-panel",
    ]);
    for (const step of TOUR_STEPS) {
